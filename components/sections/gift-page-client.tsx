...
export function GiftPageClient({ data }: { data: any }) {
    const [loadingTier, setLoadingTier] = useState<string | null>(null)

    // Fallbacks if data isn't seeded yet
    const hero = data?.hero || {
        badgeText: "A Thoughtful Gift",
        heading: "Give the Gift of Pause",
        subheading: "Know someone who deserves a moment of stillness? Gift them three months of handwritten letters, reflection prompts, and exclusive art prints — delivered to their door each month.",
        italicNote: "No account needed. No subscription for them to manage. Just beautiful months of quiet."
    }

    const howItWorks = data?.howItWorks || {
        heading: "How It Works",
        steps: [
            { stepNumber: "1", title: "Choose a Tier", description: "Select shipping based on where your recipient lives." },
            { stepNumber: "2", title: "Add a Personal Touch", description: "Write a gift message that we'll include with their first letter." },
            { stepNumber: "3", title: "They Receive Beauty", description: "A thoughtfully curated letter arrives at their door." }
        ]
    }

    const pricing = data?.pricing || {
        heading: "Choose Your Gift",
        subheading: "A one-time payment. No recurring charges.",
        tiers: []
    }

    const reassurance = data?.reassurance || {
        quoteText: "“The best gifts are the ones that give someone permission to slow down.”",
        disclaimer: "You will receive a confirmation email after purchase. The recipient's first letter will be dispatched within the current month's cycle."
    }
                ...
                <Container className="relative z-10 text-center max-w-3xl px-4 flex flex-col items-center">
                    <FadeIn>
                        <div className="inline-flex items-center gap-2 mb-8 px-5 py-2 rounded-full border border-stone-200 bg-white/60 backdrop-blur-sm">
                            ...
                            <span className="text-sm text-stone-500 font-light tracking-wide">{hero.badgeText}</span>
                        </div>
                    </FadeIn>
            ...
            <Section className="py-16 md:py-20 bg-white border-t border-stone-100">
                <Container className="max-w-4xl">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-12 md:gap-8">
                        {howItWorks.steps?.map((item: any, idx: number) => (
                            <FadeIn key={idx} className="text-center">
                                <div className="inline-flex items-center justify-center w-12 h-12 rounded-full border border-stone-200 bg-stone-50 mb-6">
                                    <span className="font-serif text-lg text-stone-700 font-light">{item.stepNumber || (idx + 1).toString()}</span>
                                </div>
